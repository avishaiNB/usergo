...
// IsExpired  return true if day of logger.dateCreated and time.Now are not eq
func (log logger) IsExpired() bool {
...
func (log *logger) Reload() {
	...
	if err == nil {
		newLogger = gokitLogger.With(newLogger,
			...
			"caller", gokitLogger.DefaultCaller,
...
func NewLogger(env string, loggerName string, minLevel AtomicLevelName) (gokitLogger.Logger, error) {
	dt := utils.DateTime{}
	dtNow := dt.Now()
